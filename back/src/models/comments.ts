...
class Comment {
  static async GetComments(filmId: Number): Promise<Object | null> {
    const text = "SELECT C.id, film_id, user_id,U.avatar,U.name, message, rating FROM main.comment as C INNER JOIN main.user as U on C.user_id=U.id WHERE C.film_id=(SELECT id FROM main.film WHERE tmdb_id=$1)"
    const values = [filmId]
    try {
  ...
  static async AddComment(filmId: Number, message: String, rating: Number, userId: string): Promise<Boolean> {
    const text = "INSERT INTO main.comment(film_id, user_id, message, rating) VALUES ((SELECT id FROM main.film WHERE tmdb_id=$1), $2, $3, $4)";
